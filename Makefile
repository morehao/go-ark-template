...
define validate_app
	@if [ -z "$(APP)" ]; then \
		...
		echo "   可用的应用程序：$$(ls ./apps 2>/dev/null | tr '\n' ' ')"; \
		exit 1; \
	fi
	@if [ ! -d "./apps/$(APP)" ]; then \
...
help:
	...
	@echo ""
	@echo "📝 Docker 镜像标签规则："
	@echo "   有 git tag  →  <tag>-<commit>   示例：demoapp:v1.0.0-abc1234"
	@echo "   无 git tag  →  <commit>         示例：demoapp:abc1234"
	@echo ""