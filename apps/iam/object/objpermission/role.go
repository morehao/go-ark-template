...
type RoleBaseInfo struct {
	...
	DataScope   iammodel.RoleDataScope `json:"dataScope" form:"dataScope"`     // 数据权限范围: all-全部 dept_and_sub-本部门及以下 dept-本部门 self-仅本人 custom-自定义
	...
	RoleType    iammodel.RoleType      `json:"roleType" form:"roleType"`       // 角色类型: custom-自定义 system-系统内置
	SortOrder   int32                  `json:"sortOrder" form:"sortOrder"`     // 排序
	Status      iammodel.RoleStatus    `json:"status" form:"status"`           // 状态: enabled-启用 disabled-停用
}
