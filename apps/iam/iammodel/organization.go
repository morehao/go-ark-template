...
type OrgEntityList []OrgEntity

const TableNameOrg = "iam_org"

func (OrgEntity) TableName() string {
	return TableNameOrg
}

func (l OrgEntityList) ToMap() map[uint]OrgEntity {
	m := make(map[uint]OrgEntity)
	for _, v := range l {
		m[v.ID] = v
	}
	return m
